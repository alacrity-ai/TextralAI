...
## What is this
Below is a raw text dump of an article discussing nexus and KnowQL.
Specifically it goes into detail about the compiling process, and then the retrieval process. It contains several images, which, due to the fact that this is a raw text dump, won't be explained, so I apologize for that.
The most important image is one which shows the example company-level artifact compiled using the 10k-SEC filings for the market-intelligence agent.

It looks roughly like:
Compiled Artifact - source - 2022/899051.txt ingested via Context Compiler
Allstate Corp - 2021 10-K summary
Ticker ALL cik 899051 fiscale_year_end 2021-12-31 exchange NYSE
Identity:
- Legal name: The allstate corporation
- Common name: Allstate
- Ticket: ALL New york Stock exchange
- CIK 899051
- Fiscale year end: December 31
Revenue and Sales:
- Total rev: 50,588M +20.7 YoY prior 41,909M
- Drivers: P&C premiums earned +13.9% (National General acquisition); net investment income +107.1%
- Allstate protection 46,030M (incl 40,454M premiums earned)
- Protection services 2,361M
- Allstate health and benefits - 2,261M
17 sections - g rounded - field level provenance [2 of 17 shown]


Below is the raw text dump of the whole article:
...
We created KRAFTBench (Knowledge Retrieval Assessment Framework for Text) to address this gap. This harness measures the accuracy, latency, and token cost of responses generated from a consistent composer model (
claude-sonnet-4-6
claude-sonnet-4-6
...
Unstructured (parsing): Unstructured connects through Box's APIs and captures each file’s content and file metadata (including ACLs). It extracts key document elements, tables, and entities from the legal contracts (
parties
parties
, 
agreement_date
